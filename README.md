...
7. 如需另一端相同 Tail，可勾選 **完成時在另一端建立鏡像 Tail**。
8. 選取 Pin Board 的對應端面。
9. 確認 Pin 預覽後按 **建立 Pin**。
10. 按 **完成** 結束操作。

### 已完成 Tail 後，再建立另一端的 Pin

選取已有 Tail 的板材，再啟動擴展，即會進入 Pin 選取模式。直接點選另一端預先擺好位置的 Pin 板，確認預覽後按 **建立 Pin**。不用重新輸入 Tail 參數，也不會再把榫頭的小面當成板寬。

V1.2.11 可從 V1.2.10 舊模型的實際面與邊讀取兩端榫形，即使舊版只儲存了第一端的資料，仍可辨識另一端。Pin 板須等厚、同寬，並位於 90° 的實際組裝位置。

參數、計算結果及操作按鈕已合併至不需捲動的版面。更新後請完全關閉並重新啟動 SketchUp。
